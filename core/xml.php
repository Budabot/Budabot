...
class org extends xml {
	...
	function __construct($org_id = 0, $rk_num = 0, $cache = null, $force_update = false){
		...
		//organisation lookup
        $this->lookup($org_id, $rk_num, $cache, $force_update);
	} //end of contructor

    //the organisation lookup function
	function lookup($org_id, $rk_num, $cache, $force_update) {
	 	global $vars;
		$data_found = false;
		$data_save = false;
		
		//Check if a xml file of the person exists and if it is uptodate
		if (!force_update && file_exists("$cache/$org_id.$rk_num.xml")) {
	        $mins = (time() - filemtime("$cache/$org_id.$rk_num.xml")) / 60;
            $hours = floor($mins/60);
            ...
            if (($hours < 24 && $vars["my guild id"] != $org_id) || ($hours < 6 && $vars["my guild id"] == $org_id)) {
             	$orgxml = file_get_contents("$cache/$org_id.$rk_num.xml");
				if (xml::spliceData($orgxml, '<id>', '</id>') == $org_id) {
					$data_found = true;
				} else {
					$data_found = false;
					unset($orgxml);
					@unlink("$cache/$org_id.$rk_num.xml");
				}
			}
        }
        ...
        if (!$data_found) {
			$orgxml = xml::getUrl("http://people.anarchy-online.com/org/stats/d/$rk_num/name/$org_id/basicstats.xml", 30);
			if (xml::spliceData($orgxml, '<id>', '</id>') == $org_id) {
				$data_found = true;
				$data_save = true;
			} else {
		...
		if (!$data_found && file_exists("$cache/$org_id.$rk_num.xml")) {
			$orgxml = file_get_contents("$cache/$org_id.$rk_num.xml");
        ...
        forEach ($members as $amember) {
			...
			$this->members[$name]->prof_title     = xml::spliceData($amember, '<profession_title>', '</profession_title>');
			...
			$this->members[$name]->guild_id       = $this->org_id;
			$this->members[$name]->guild          = $this->orgname;
            $this->members[$name]->guild_rank     = xml::spliceData($amember, '<rank_name>', '</rank_name>');
            $this->members[$name]->guild_rank_id  = xml::spliceData($amember, '<rank>', '</rank>');
			$this->members[$name]->dimension      = $rk_num;
			$this->members[$name]->source         = 'org_roster';
			
			Player::update($this->members[$name]);
		}

		//if a new xml file was downloaded, save it
		if ($data_save) {
	        $fp = fopen("$cache/$org_id.$rk_num.xml", "w");
	        fwrite($fp, $orgxml);
	        fclose($fp);
	    }
	} //end lookup
