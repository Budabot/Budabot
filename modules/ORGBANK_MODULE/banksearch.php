...
if (preg_match("/^banksearch (.+) (.+)$/i", $message, $arr) || preg_match("/^banksearch (.+)$/i", $message, $arr) ) {
 ...
 //So... lets look for a bank.
		$db->query("SELECT * FROM $table WHERE `bankowner` = '$person_to_find' ");
		...
		IF ($items_found > 1) {
			...
			if ($comment){
				$msg .= ("<grey>$person_to_find <green>comments: $comment<end>\n");
			}
			if($online_status == 1){
				$msg .=("<grey>$person_to_find <green> is currently:<green> Online<end> \n");
			}ELSE{
				$msg .=("<grey>$person_to_find <green>is currently:<red> Offline<end> \n");
			}
