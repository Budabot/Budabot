...
// if( eregi("^bank[D,R,U,H] ([0-9]+)\/([0-9]+)\/([0-9]+)\/(.+)\/(.+)?$", $message, $arr)) {
if( eregi("^bank[M,D,P] ([0-9]+)\/([0-9]+)\/([0-9]+)\/(.+)\/(.+)?$", $message, $arr)) {
				...
				$db->query("SELECT * FROM $table WHERE `lowID` = '$lowID' AND `highID` = '$highID' AND `ql` = '$itemQL' AND `bankowner` = '$sender'");
				...
				if ($db->numrows() > 0) { // Found it
					...
					if ($quantity == 0) {
						$db->query("DELETE FROM $table WHERE `lowID` = '$lowID' AND `highID` = '$highID' AND `ql` = '$itemQL' AND `bankowner` = '$sender'");
						$msg = "Removed: [QL $itemQL] <a href=\"itemref://$lowID/$highID/$itemQL\">$itemname</a> ";
					} else {
						$db->query("UPDATE $table SET $setstr WHERE `lowID` = '$lowID' AND `highID` = '$highID' AND `ql` = '$itemQL' AND `comment` = '$comment' AND `bankowner` = '$sender'");
