...
global $raid_points;
global $raidloot;
if(eregi("^raidstart (.+)$", $message, $arr)) {
	if($this->vars["raid_status"] != "") {
		$msg = "<red>A raid is already started.<end>";
		bot::send($msg);
		return;
	}
	...
	$this->vars["raid_status"] = $raid_settings->raid_name;
  	...
  	while($row = $db->fObject())
  		$raidlist[$row->name] = $row->level;
  		
  	$raid_points = false;
	$this->vars["raid_loot_cat"] = 0;
	$this->vars["raid_loot_index"] = 0;
	$this->vars["raid_pts"] = $raid_settings->pts;
	$this->vars["raid_bid_cost"] = $raid_settings->bid_cost;
