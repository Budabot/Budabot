...
if(eregi("^setpf$", $message)) {
  	bot::savesetting("pftimer", time());
	$msg = "Pftimer has been updated.";
  	bot::send($msg, $sender);  	
} elseif(eregi("^setpf 0$", $message)) {
	...
	bot::send("PFtimer is now unlocked.", $sender);
	...
	$this->settings["pftimer"] += $ctime;
  	bot::savesetting("pftimer", $this->settings["pftimer"]);
	$msg = "Pftimer has been updated.";
  	bot::send($msg, $sender);
