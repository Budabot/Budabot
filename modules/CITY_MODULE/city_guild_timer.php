...
if ($db->numrows() != 0) {
    ...
    if ($row->action == "off") {
        if (((time() - $row->time) >= 60*60) && ((time() - $row->time) < 61*60)) {
            bot::send("Shields have been disabled one hour ago. It is now possible to enable it again.", "guild");
		}
    } else if ($row->action == "on") {
        if (((time() - $row->time) >= 60*60) && ((time() - $row->time) < 61*60)) {
            bot::send("Shields have been enabled one hour ago. Alien attacks can be again initiated now.", "guild");
		}
    }
}
?>
