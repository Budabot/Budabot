...
if (preg_match("/^check$/i", $message) || preg_match("/^check all$/i", $message)) {
	...
	$db->query("SELECT name FROM online o LEFT JOIN players p ON o.charid = o.charid WHERE added_by = '<myname>' AND channel_type = 'priv'");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		$content .= " \\n /assist $row->name";
	}
	...
	$db->query("SELECT * FROM online o LEFT JOIN players p ON o.charid = o.charid WHERE added_by = '<myname>' AND channel_type = 'priv' ORDER BY `profession` DESC");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		$prof[$row->profession] .= " \\n /assist $row->name";
	}
	...
	$db->query("SELECT * FROM online o LEFT JOIN players p ON o.charid = p.charid WHERE added_by = '<myname>' AND channel_type = 'priv' ORDER BY `guild` DESC");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		if ($row->guild == "") {
			$org["Non orged"] .= " \\n /assist $row->name";
