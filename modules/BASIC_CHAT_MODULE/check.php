...
if (preg_match("/^check$/i", $message) || preg_match("/^check all$/i", $message)) {
	...
	$db->query("SELECT o.name, p.profession FROM online o LEFT JOIN players p ON (o.name = o.name AND p.dimension = '<dim>') WHERE added_by = '<myname>' AND channel_type = 'priv' ORDER BY `profession` DESC");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		$prof[$row->profession] .= " \\n /assist $row->name";
	}
	...
	$db->query("SELECT o.name, p.guild FROM online o LEFT JOIN players p ON (o.name = p.name AND p.dimension = '<dim>') WHERE added_by = '<myname>' AND channel_type = 'priv' ORDER BY `guild` DESC");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		if ($row->guild == "") {
			$org["Non orged"] .= " \\n /assist $row->name";
